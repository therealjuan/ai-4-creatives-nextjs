...
export default function FlipButton(props: FlipButtonProps) {
    ...
    return (
        <button className={`flippable-button h-12 cursor-pointer w-48 text-xs text-white hover:text-black ${flipped ? 'flipped' : ''} bg-black`}>
            <div className="front text-xl lg:text-[1.6vh] absolute top-0 left-0 w-full h-full flex items-center p-4 justify-start hover:bg-green-500 " onClick={handleClick}>{t('shareLove')} <Image src={shareLoveImg} className='absolute' loading="lazy" alt={t('shareLove')} /></div>
